...
.stat-number.neuf { color: #1c9a2fff; }
.stat-number.reforme { color: #b83131ff; }
...
/* === ALL OTHER STYLES (FORM, MODAL, ETC.) GO HERE AS THEY WERE === */
/* ... (The rest of the CSS like .form-view-container, .modal-container etc. remains unchanged) ... */
...
@media (max-width: 1023px) 
{
  ...
  /* HEADER TABLET - Corrected to mimic mobile layout */
  .header {
    display: grid;
    grid-template-columns: auto 1fr auto auto; /* 4 columns for back, title, notifications, menu */
    grid-template-rows: auto auto; /* 2 rows for header items and search bar */
    gap: 8px;
    align-items: center;
    margin-bottom: 14px;
    width: 100%;
  ...
  .header-right .search-input { display: none; } /* Hide desktop search */
  
  /* This element is now logically separate from header-right on mobile/tablet */
  ...
  .search-input { width: 100%; box-sizing: border-box; font-size: 14px; }
  ...
  /* CORRECTIONS POUR LES FILTRES SUR TABLETTE */
  .actions-container { 
    flex-direction: column; 
    align-items: stretch; 
    gap: 12px; 
  ...
  /* RÉDUCTION DE LA TAILLE DES AUTRES COMPOSANTS POUR AGRANDIR LE TABLEAU */
  .stats-container { 
    margin-bottom: 12px;
    gap: 10px;
  }
...
@media (max-width: 767px) {
  ...
  /* HEADER MOBILE - Keeping the corrected grid */
  .header {
    grid-template-columns: auto 1fr auto auto; /* 4 cols */
  ...
  .add-button, .pdf-button, .category-button, .delete-all-button { 
    justify-content: center; 
    width: 100%;
  }

  /* CORRECTIONS POUR LES FILTRES SUR MOBILE */
  .filters-wrapper { 
    display: flex;
    flex-direction: column;
    gap: 8px;
  ...
  /* RÉDUCTION MAXIMALE DES AUTRES COMPOSANTS */
  .stats-container { 
    margin-bottom: 10px;
    gap: 8px;
  }
  ...
  /* AGRANDISSEMENT MAXIMAL DU TABLEAU */
  .table-container { 
    min-height: 55vh;
    flex-grow: 1;
  }
...
/* Amélioration de la visibilité des filtres actifs */
...
export default function StocksScreen() {
  ...
  // Ajouter avec les autres états de filtre
...
const processedData = useMemo(() => {
  ...
  if (filterDate) {
    list = list.filter(s => 
      s.date_MiseAJour && 
      String(s.date_MiseAJour).slice(0, 10) === filterDate
    )
  }
  ...
  return list
}, [allStocks, searchQuery, filterStatus, filterCategory, filterDate, filterMonth, filterYear, sortOrder])
...
<div className="dropdown">
  ...
  {dropdownOpen === 'category' && (
    <div className="dropdown-content">
      ...
      <button onClick={() => { setFilterCategory(null); setDropdownOpen(null); setCategorySearch(''); }}>
        Toutes
      </button>
...
>
  {`${l.id_livraison} - ${((l.designation || '').split(' - ')[0] || l.designation || '')}`}
</button>
